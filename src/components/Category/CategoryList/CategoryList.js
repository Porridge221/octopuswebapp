...
function CategoryList() {
  ...
  const fetchData = () => {
    fetch("http://localhost:8000/products/catalog/1", {method: 'GET', headers: {'Content-Type': 'application/json', 'Authorization': initData}})
      .then(response => {
  ...
  return (
    result.length > 0 ? (
      <div className={styles.root}>
          {result.map(producer => ( 
            <CategoryGroup key={producer.id} producer={producer}/>
          ))}
      </div>
    ) : (<div>Нет данных</div>)
    // items.length > 0 ? (
